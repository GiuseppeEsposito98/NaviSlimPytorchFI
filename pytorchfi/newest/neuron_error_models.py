...
from pytorchfi.newest import core
...
class single_bit_flip_func(core.FaultInjection):
    ...
    def _flip_bit_signed(self, orig_value, max_value, bit_pos):
        ...
        logger.info(extra=extra, msg=f"Original Value: {orig_value}")

        quantum = int((orig_value / max_value) * ((2.0 ** (total_bits - 1))))
        twos_comple = self._twos_comp_shifted(quantum, total_bits)  # signed
        logger.info(extra=extra, msg=f"Quantum: {quantum}")
        logger.info(extra=extra, msg=f"Twos Couple: {twos_comple}")

        # binary representation
        bits = bin(twos_comple)[2:]
        logger.info(extra=extra, msg=f"Bits: {bits}")
        ...
        logger.info(extra=extra, msg=f"Sign extend bits {bits}")
        ...
        logger.info(extra=extra, msg=f"New bits: {bits_str_new}")

        # GPU contention causes a weird bug...
        if not bits_str_new.isdigit():
            logger.info(extra=extra, msg="Error: Not all the bits are digits (0/1)")
        ...
        logger.info(extra=extra, msg=f"Out: {out}")

        # get FP equivalent from quantum
        new_value = out * ((2.0 ** (-1 * (total_bits - 1))) * max_value)
        logger.info(extra=extra, msg=f"New Value: {new_value}")
        ...
        if type(corrupt_conv_set) is list:
            ...
            for i in inj_list:
                if(i<output.shape[0]):
                    ...
                    logger.info(extra=extra, msg=f"Random Bit: {rand_bit}")
